...
Non-negotiable rules (CODING_STANDARDS.md):
...
5. Idempotent — same idempotency_key must not double-send
"""
...
from app.services.esp_client import (
...
def _public_base_url() -> str:
    ...
    # Prefer explicit PUBLIC_APP_URL; fall back to first CORS origin
...
def resolve_recipient_email(
    session: Session, message: Message, lead: Lead
) -> tuple[str, Optional[UUID]]:
    """
    Resolve the email address to send to.
    Prefer message.contact_id → contact.value; else first email contact on business;
    else business.phone is not used for email.
    """
    ...
    raise SendBlockedError(
        "NO_RECIPIENT",
        "No email contact found for this lead/business. Attach a contact_id or add an email contact.",
    )
...
def _check_volume_caps(session: Session, organization_id: UUID) -> None:
    ...
    # Approximate week start (UTC Monday)
    week_ago = now - __import__("datetime").timedelta(days=now.weekday())
    week_ago = week_ago.replace(hour=0, minute=0, second=0, microsecond=0)

    daily = _count_sends_since(session, organization_id, day_ago)
    if daily >= settings.ESP_DAILY_SEND_CAP_PER_ORG:
        raise SendBlockedError(
            "DAILY_CAP",
            f"Daily send cap ({settings.ESP_DAILY_SEND_CAP_PER_ORG}) reached for this organization",
        )
    weekly = _count_sends_since(session, organization_id, week_ago)
    if weekly >= settings.ESP_WEEKLY_SEND_CAP_PER_ORG:
        raise SendBlockedError(
            "WEEKLY_CAP",
            f"Weekly send cap ({settings.ESP_WEEKLY_SEND_CAP_PER_ORG}) reached for this organization",
        )


def _default_from_address(organization_id: UUID) -> tuple[str, str]:
    """
    Platform-assigned isolated subdomain style from-address.
    Full SPF/DKIM verification lands in Phase 5.5; for now we use config domain.
    """
    settings = get_settings()
    domain = settings.ESP_PLATFORM_SENDING_ROOT_DOMAIN or "mail.localhost"
    # Short org id prefix for isolation label
    short = str(organization_id).replace("-", "")[:8]
    from_email = f"outreach@{short}.{domain}" if domain != "mail.localhost" else f"noreply@{domain}"
    # Console/dev: use a simple local address
...
def send_approved_message(session: Session, message_id: UUID) -> Message:
    """
    Authoritative send path. Must only be called from the send worker
    (or tests). Re-validates every safety rule inside this function.
    """
    message = session.execute(
        select(Message)
        .where(Message.id == message_id)
        .options(
            selectinload(Message.lead)
    ...
    # 1) Status must be approved — never trust the caller
    if message.status != "approved":
        raise SendBlockedError(
            "NOT_APPROVED",
            f"Refusing to send: status is '{message.status}', expected 'approved'",
        )
    ...
    # 0) Sending identity: SPF+DKIM verified and not abuse-paused
    from app.services.sending_identity_service import (
        SendingIdentityError,
        assert_can_send_sync,
        record_send_sync,
    )
    # Subscription / trial must allow outbound
    ...
    # 3) Suppression re-check at send time
    if _is_suppressed(session, organization_id, to_email):
        raise SendBlockedError(
            "SUPPRESSED_CONTACT",
            f"Contact {to_email} is on the organization suppression list",
        )

    # 4) Volume caps
    _check_volume_caps(session, organization_id)

    # 5) Unsubscribe token + mandatory link injection
    ...
    message.status = "sent"
    message.sent_at = now
    message.content = body  # persist injected unsubscribe if we added it
    ...
    # Move lead stage to contacted on first successful send
    ...
    logger.info(
        "Message %s sent via %s to %s (esp_id=%s)",
        message.id,
        result.provider,
        to_email,
...
def _default_subject(lead: Lead) -> str:
    name = ""
    if lead.business and lead.business.name:
