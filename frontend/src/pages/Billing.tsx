...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize ${classes}`}
    >
      {formatStatus(status)}
    </span>
  )
    ...
    <div>
      <div className="flex items-center justify-between text-xs mb-1.5">
        ...
        <span className="text-slate-500">
          ...
          {safeLimit > 0 ? safeLimit.toLocaleString() : 'Unlimited'}
        </span>
      </div>
...
export default function Billing() {
  ...
  useEffect(() => {
    ...
    if (params.get('success') === '1') {
      setNotice(
        'Payment completed. Your subscription will update after Stripe confirms the payment.'
      )
    }
    ...
    const load = async () => {
      try {
        ...
        setError(
          (e as Error).message || 'Failed to load billing information'
        )
      } finally {
        setLoading(false)
      }
  ...
  /*
   * Try several common usage shapes so this page remains compatible
   * with different /usage API response formats.
   */
  const campaignsUsed =
    usage?.campaigns_used ??
    usage?.campaigns ??
    usage?.current?.campaigns ??
    0
  ...
  const campaignLimit =
    caps.max_campaigns_per_month ??
    caps.max_campaigns ??
    0

  const leadLimit =
    caps.max_leads_per_month ??
    caps.max_leads ??
    0
    ...
    <div className="max-w-4xl">
          ...
          <section className="bg-white border rounded-xl p-5 mb-6">
            ...
            {sub.trial_end && (
              <div className="mt-4 p-3 bg-blue-50 rounded-lg text-sm text-blue-700">
                <strong>Trial:</strong> Your trial ends on{' '}
          ...
          <section className="bg-white border rounded-xl p-5 mb-8">
            <div className="mb-4">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Usage resets according to your billing period.
              </p>
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {PLANS.map((plan) => {
          ...
          return (
            ...
            >
              {isCurrent && (
                <span className="absolute top-4 right-4 text-xs font-medium bg-brand-100 text-brand-700 px-2 py-1 rounded-full">
                  Current plan
                </span>
              )}
                ...
                className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium transition ${
                  isCurrent
                    ...
                    : plan.id === 'starter'
                      ...
                      : 'border border-brand-600 text-brand-700 hover:bg-brand-50 disabled:opacity-50'
                }`}
              >
                {isCurrent
                  ? 'Current Plan'
      ...
      <div className="mt-6 p-4 bg-slate-50 border rounded-xl">
        <p className="text-xs text-slate-600 leading-relaxed">
          Payments are securely processed through Stripe. Your plan is
          updated after the payment webhook is successfully received by
          the server. If your plan does not update immediately after
          payment, refresh this page after a few moments.
        </p>
      </div>
    </div>
  )
